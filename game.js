...
class TDScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.bestWave = SAVE_DATA.bestWave || 0;
    this.bestLevel = SAVE_DATA.bestLevel || 0;
  ...
  triggerGameOver() {
    ...
    const isNewRecord = this.wave > this.bestWave;
    if (isNewRecord) { this.bestWave = this.wave; this.bestLevel = this.level; }
    this.saveProgress();
  ...
  /**
   * Game-over → restart is a natural "logical pause" point for a fullscreen ad
   * (per platform ad-placement requirements). If no ad is available/configured,
   * we fall straight through to restarting.
   */
  restartWithAd() {
    if (typeof Bridge === 'undefined') { this.scene.restart(); return; }
    let done = false;
    const proceed = () => { if (done) return; done = true; this.scene.restart(); };
    const shown = Bridge.showInterstitial({ onClose: proceed, onError: proceed, onOffline: proceed });
    if (!shown) proceed();
  }
...
if (typeof Bridge !== 'undefined') {
  Bridge.init({ onPause: pauseGame, onResume: resumeGame }).then(async () => {
    Bridge.notifyLoadingStart();
    try {
      const cloud = await Bridge.loadCloudData();
